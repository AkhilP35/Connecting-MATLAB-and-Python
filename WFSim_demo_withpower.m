...
addpath(fullfile(wfsim_dir, 'layoutDefinitions')); % Folder with predefined wind farm layouts
addpath(fullfile(wfsim_dir, 'controlDefinitions')); % Make use of a predefined timeseries of control inputs
addpath(fullfile(wfsim_dir, 'solverDefinitions')); % Folder with model options, solver settings, etc.
...
figure;
hPowerPlot = plot(zeros(1, NN), 'LineWidth', 2); % Preallocate time-series plot
xlabel('Timestep');
ylabel('Total Power (W)');
title('Real-Time Total Power');
grid on;
xlim([1, NN]);
ylim([0, 6e8]); % Adjust the Y-axis limit based on realistic power ranges for your turbines
...
while sol.k < NN
    ...
    %NEW: Calculate power output for each turbine at this timestep
    for i = 1:Wp.turbine.N
    ...
    totalPowerHistory(sol.k) = sum(powerOutput(:, sol.k));
    set(hPowerPlot, 'YData', totalPowerHistory); % Dynamically update the YData
    drawnow; % Force the plot to update immediately
    xlim([1, NN]); % X-axis spans all timesteps
    ylim([0, 6e8]); % Y-axis range depends on your turbine model's power output
